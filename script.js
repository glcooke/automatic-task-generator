...
function system_convert(dec, start_system, end_system) {
  return parseInt(dec, start_system)
    .toString(end_system)
    .toUpperCase();
}
...
function printInTable(divID, printText, i, answer) {
  var text = `
        <div><p style = "text-align: left"> Задание ${i + 1}:<p>
        
        <div id="generated${i}" style="margin-bottom: 5px;text-align: left;">${printText}</div>
        <div class="block">
        <br>
        <button class="hide" style="float: left;" name="show">Ответ к заданию ${i +
          1}: </button>
		<br><br>
...
function printTables(divID, printText, i, answer1, answer2) {
  ...
  document.getElementById(divID).innerHTML += `
		...
		<br><br>
    <div class="extremum-slide" style="display: none">
    <div class="answer" style="margin-bottom: 5px;  text-align: left;">Ответ: ${answer1}, путь: ${answer2}.</div>
		</div>
    <hr>
    </div>`;
  $("button").click(function() {
...
function create_test() {
  document.getElementById("rightwindow").innerHTML = `
    <h1>Выбор задач для контрольной</h1><br>
...
function test_ready() {
  ...
  if (
    ...
    alert("Не выбрано ни одного задания!");
  } else {
    if (
      (Math.sign(task_1_easy) == 0 || Math.sign(task_1_easy) == 1) &&
      (Math.sign(task_1_medium) == 0 || Math.sign(task_1_medium) == 1) &&
      (Math.sign(task_1_hard) == 0 || Math.sign(task_1_hard) == 1) &&
      (Math.sign(task_2_easy) == 0 || Math.sign(task_2_easy) == 1) &&
      (Math.sign(task_2_medium) == 0 || Math.sign(task_2_medium) == 1) &&
      (Math.sign(task_2_hard) == 0 || Math.sign(task_2_hard) == 1) &&
      (Math.sign(task_3_easy) == 0 || Math.sign(task_3_easy) == 1) &&
      (Math.sign(task_3_medium) == 0 || Math.sign(task_3_medium) == 1) &&
      (Math.sign(task_3_hard) == 0 || Math.sign(task_3_hard) == 1)
    ) {
      document.getElementById("rightwindow").innerHTML = "";
      for (var j = 0; j < variants; j++) {
        ...
        var count = 0;
        document.getElementById("rightwindow").innerHTML += `<h1>Вариант ${j +
          1}</h1><hr>`;
        ...
        for (var i = 0; i < task_3_easy; i++) {
          generate_algo_1(count);
          count++;
        }
        for (var i = 0; i < task_3_medium; i++) {
          generate_algo_2(count);
          count++;
        }
        for (var i = 0; i < task_3_hard; i++) {
          generate_algo_3(count);
          count++;
        }
        
      }
    } else {
      alert("Что-то не так с параметрами задач!");
    }
  }
}
